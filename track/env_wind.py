...
def gen_wind_mean_cov():
    ...
    # Combine intermediate files safely
    with input._open_fns(out_fns) as ds:
        ds.load()  # load all data into memory
        da = ds['wnd_stats'].copy()  # ensure no lingering references
    ...
    var_dict = {var_names[i]: da[:, i, :, :].copy() for i in range(len(var_names))}

    # Write final NetCDF safely
    with xr.Dataset(data_vars=var_dict) as ds_e:
    ...
    del da, var_dict
    gc.collect()
    time.sleep(0.1)  # small pause for Windows file locks

    # Remove intermediate files safely
    for fn in out_fns:
        try:
            os.remove(fn)
        except PermissionError:
            print(f"Warning: could not remove {fn}, file may still be in use")
